...
// Classe abstrata ProcessFactory
abstract class AbstractFactory {
    abstract Process createProcess(int id, int arrivalTime, int burstTime);
    abstract Page createPage(int processId, int pageId);
}

// Fábrica concreta para criar Process e Page
class SchedulerFactory extends AbstractFactory {
    ...
    @Override
    Page createPage(int processId, int pageId) {
        return new Page(processId, pageId);
    }
}
...
public class SJFInterface extends JFrame {
    ...
    public SJFInterface(AbstractFactory factory) {
        this.factory = factory;

        setTitle("SJF Scheduler with Memory Allocation");
        setSize(800, 600);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setLayout(new BorderLayout());

        // Painel para entrada de dados de processos
        ...
        txtNumFrames = new JTextField();
        memoryPanel.add(txtNumFrames);

        memoryPanel.add(new JLabel("Tamanho da Página:"));
        txtPageSize = new JTextField();
        memoryPanel.add(txtPageSize);

        JButton btnAllocateMemory = new JButton("Alocar Memória");
        memoryPanel.add(btnAllocateMemory);
        add(memoryPanel, BorderLayout.SOUTH);

        // Ação para adicionar processos
        btnAddProcess.addActionListener(e -> addProcess());

        // Ação para calcular SJF
        btnCalculate.addActionListener(e -> calculateSJF());

        // Ação para alocar memória
        btnAllocateMemory.addActionListener(e -> allocateMemory());
    }
    ...
    private void addProcess() {
        try {
            ...
            Process process = factory.createProcess(++processCount, arrivalTime, burstTime);
    ...
    private void calculateSJF() {
        ...
        // Ordenar os processos por tempo de burst
    ...
    private void allocateMemory() {
        try {
            ...
            for (int i = 0; i < frameCount; i++) {
                ...
                Page page = factory.createPage(processId, pageId);
                frames[i] = page;
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            AbstractFactory factory = new SchedulerFactory();
            SJFInterface frame = new SJFInterface(factory);
